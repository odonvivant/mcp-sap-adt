...
| Status | Tools |
|---|---|
| **Live-verified** | discovery, search, DDIC metadata + table contents (with `sqlQuery` filter), usage references, package contents, revisions, transport info/create/**release**, ATC run/worklist, syntax check, code completion, object source read/write (incl. class includes like `testclasses`), object create/lock/unlock/activate/delete, **refactor (rename/extract-method)**, **unit test run**, traces list |
